...
def encrypt(key,word_to_encrypt):
    cipher = bytearray()
    for i in range(len(word_to_encrypt)):
        cipher.append(ord(word_to_encrypt[i])^ ord(key[i%len(key)]))
    return cipher
    #print(cipher)
...
def main():
    ...
    key = "ICE"
    word_to_encrypt = "Burning 'em, if you ain't quick and nimble I go crazy when I hear a cymbal"

    encrypted = encrypt(key,word_to_encrypt)
    print(encrypted.hex())
